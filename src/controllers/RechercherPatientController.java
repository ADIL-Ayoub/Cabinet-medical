...
public class RechercherPatientController implements Initializable{
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		try {
			...
			FilteredList<PatientModel> filteredData = new FilteredList<>(data, b -> true);
			champTxt.textProperty().addListener((observable, oldValue, newValue) -> {
				filteredData.setPredicate(PatientModel -> {
					if(newValue.isEmpty() || newValue.isBlank() || newValue == null) {
						return true;
					}
					String motChercher = newValue.toLowerCase();
					if(PatientModel.getNom().toLowerCase().indexOf(motChercher) > -1) {
						return true;
					}else if(PatientModel.getPrenom().toLowerCase().indexOf(motChercher) > -1) {
						return true;
					}else if(PatientModel.getCin().toLowerCase().indexOf(motChercher) > -1) {
						return true;
					}else {
						return false;
					}
